...
//Set
// 요소의 중복을 허용하지 않는 자료구조 = 동일한 값은 한 번만 저장
// 요소의 순서(인덱스) 없는 자료구조
// 자바 스크립트에서는 배열을 가지고 Set을 만듬

const set = new Set([1, 1, 2, 2, 3, 3, 4, 5]);
console.log(set); //Set(5) { 1, 2, 3, 4, 5 }      중복된 값은 하나로 침
...
// console.log(new Set([1, 2, 3]).clear());//undefined
...
//Set은 이터러블: for~or 순회가능, 스프레드 가능, 구조 분해 할당 가능
...
console.log(setB.difference(setB));
console.log(setB.difference2(setB));

//Map
// 키와 값(엔트리)으로 구성된 자료구조
// 엔트리의 순서(인덱스)는 없음
// 키는 중복 불가, 값은 중복 가능
// 자바 스크립에서 Map은 2차원배열을 이용해 만큼
...
//keys(), values(), entries()
for(const key of map2.key()){
    console.log(key);
}
for(const value of map2.values()){
    console.log(value);
}
for(const entry of map2.entry()){
    console.log(entry);
}